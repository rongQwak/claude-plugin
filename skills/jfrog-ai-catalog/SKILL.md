...
- **Skill listing is governance-filtered by default**: `--list-skills` and
  `--list-skill-versions` both always get `--allowed-only` unless the user
  explicitly asks to see blocked/all skills or repos. Agent Guard is unfiltered
  when the flag is omitted, so passing it is what makes a listing
  governance-filtered — see `discovering-skills.md`. Each `locations[]` entry
  from `--list-skill-versions`
